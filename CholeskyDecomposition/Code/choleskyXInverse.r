...
cSource <- "
  ...
  // [[Rcpp::export]]
  NumericMatrix choleskyXInverse(NumericMatrix R, int nCore=0) {
    ...
    // Done!
    // Copy vectors of computed inverse to result matrix
    for(j=0; j<p; j++)
      memcpy(&iXOut(0,j), iX[j], sizeof(double)*p);
